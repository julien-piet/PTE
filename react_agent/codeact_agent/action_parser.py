...
try:
    ...
    class AgentDelegateAction(Action):  # type: ignore[no-redef]
        def __init__(self, agent='', inputs=None): self.agent = agent; self.inputs = inputs or {}
    ...
    class CmdRunAction(Action):  # type: ignore[no-redef]
        def __init__(self, command='', thought=''):
            self.command = command; self.thought = thought
    class IPythonRunCellAction(Action):  # type: ignore[no-redef]
        def __init__(self, code='', thought='', kernel_init_code=''):
            self.code = code; self.thought = thought

    class ResponseParser:  # type: ignore[no-redef]
        def parse(self, response): raise NotImplementedError
        def parse_response(self, response): raise NotImplementedError
        def parse_action(self, action_str): raise NotImplementedError

    class ActionParser:  # type: ignore[no-redef]
        def check_condition(self, action_str): raise NotImplementedError
        def parse(self, action_str): raise NotImplementedError
