...
for WORKER_ID in $(seq 1 "$TOTAL_WORKERS"); do
  ...
  SHOPPING_HEALTH_REMOTE=$((BASE + 1))
  ADMIN_REMOTE=$((BASE + 2))
  ADMIN_HEALTH_REMOTE=$((BASE + 3))
  REDDIT_REMOTE=$((BASE + 4))
  REDDIT_HEALTH_REMOTE=$((BASE + 5))
  GITLAB_REMOTE=$((BASE + 6))
  GITLAB_HEALTH_REMOTE=$((BASE + 7))

  if $FORWARD_SHOPPING; then
    CMD+=(-L "${SHOPPING_REMOTE}:127.0.0.1:${SHOPPING_REMOTE}")
    CMD+=(-L "${SHOPPING_HEALTH_REMOTE}:127.0.0.1:${SHOPPING_HEALTH_REMOTE}")
  fi

  if $FORWARD_ADMIN; then
    CMD+=(-L "${ADMIN_REMOTE}:127.0.0.1:${ADMIN_REMOTE}")
    CMD+=(-L "${ADMIN_HEALTH_REMOTE}:127.0.0.1:${ADMIN_HEALTH_REMOTE}")
  fi

  if $FORWARD_REDDIT; then
    CMD+=(-L "${REDDIT_REMOTE}:127.0.0.1:${REDDIT_REMOTE}")
    CMD+=(-L "${REDDIT_HEALTH_REMOTE}:127.0.0.1:${REDDIT_HEALTH_REMOTE}")
  fi

  if $FORWARD_GITLAB; then
    CMD+=(-L "${GITLAB_REMOTE}:127.0.0.1:${GITLAB_REMOTE}")
    CMD+=(-L "${GITLAB_HEALTH_REMOTE}:127.0.0.1:${GITLAB_HEALTH_REMOTE}")
  fi
done
