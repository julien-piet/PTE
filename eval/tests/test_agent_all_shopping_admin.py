# eval/tests/test_agent_all_shopping_admin.py
#
# Integration tests: run the agent on every shopping_admin task in
# raw_webarena_tasks_all_shopping_admin.json (182 tasks, both string_match and
# program_html eval types).
#
# Unlike the GitLab tests there are no Docker workers — shopping_admin runs against
# a single fixed server.  The shared session-scoped ``agent_runner`` from
# conftest.py is used directly.
#
# Run all 182 tasks:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py -v
#
# Smoke test (first 5 tasks):
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py --task-limit 5 -v -s
#
# Single task by ID:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py -k "task_157" -v -s
#
# Only string_match tasks:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py -k "string_match" -v
#
# Save results to a JSON log:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py -v --output shopping_admin_results.json
#
# Force-reset shopping_admin state before every write task:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py -v --force-reset
#
# Plug in a custom agent runner:
#   python3 -m pytest eval/tests/test_agent_all_shopping_admin.py \
#       --agent-runner my_agent_runner.MyAgentRunner -v -s
#
# Task breakdown (182 total):
#   string_match  :  91  (88 original + 3 converted from url_match)
#   program_html  :  91  (66 original + 25 converted from url_match+program_html)
#
# read_only flag:
...
TASK_FILE = Path(__file__).parent / "raw_webarena_tasks_all_shopping_admin.json"
...
def _task_id(task: Dict[str, Any]) -> str:
    return f"task_{task['task_id']}_shopping_admin"
...
def test_agent_accomplishes_shopping_admin_task(
    agent_runner,
    session_event_loop,
    result_log,
    request,
    ...
    """
    Assert the agent correctly accomplishes a single shopping_admin task.
