...
class PlanningAgent:
    ...
    # ------------------------------------------------------------------
    # Step 5b: LLM filters all possibly relevant endpoints (broad pass)
    # ------------------------------------------------------------------
    async def _filter_relevant_endpoints(self, task: str, endpoints: List[EndpointInfo]) -> List[EndpointInfo]:
        endpoint_list = "\n".join(
        ...
        api_hints_section = (
            "\nAPI context \u2014 use these rules to determine what prerequisite lookups may be needed:\n"
            + "\n\n".join(relevant_hints) + "\n"
        ...
        prompt = (
            ...
            + api_hints_section
            + "Select all endpoints that could plausibly be needed to complete this task.\n\n"
            "Rules:\n"
            "- Be inclusive: if an endpoint might be relevant as a direct action OR as a prerequisite lookup, include it.\n"
            "- Use the API context above to identify required lookups (e.g. if a task mentions a user by name, include user search endpoints).\n"
            "- Do NOT do dependency chain reasoning here \u2014 that happens in a later step.\n"
            "- Do NOT worry about whether inputs are satisfied \u2014 just assess topical relevance.\n"
            "- When in doubt, include the endpoint.\n\n"
            'Respond with ONLY valid JSON:\n'
            '{"selected_indices": [0, 1, 4, 7], "reasoning": "Endpoint 0 looks up users by name (needed to resolve the username to an ID). Endpoint 1 is the main action. ..."}'
        )
        self._debug_print("_filter_relevant_endpoints", prompt=prompt)
        agent = Agent(self.llm, output_type=str)
        result = await agent.run(prompt)
        response = result.output
        self._debug_print("_filter_relevant_endpoints", response=response)
        decoder = json.JSONDecoder()
        data = None
        for i, ch in enumerate(response):
            if ch == "{":
        ...
        selected = [endpoints[i] for i in indices if 0 <= i < len(endpoints)]

        if self.debug_responses:
            reasoning = data.get("reasoning", "")
            print("\n[PlanningAgent] _filter_relevant_endpoints SELECTED ENDPOINTS:")
            if reasoning:
                print(f"  Reasoning: {reasoning}")
            print(f"  Count: {len(selected)} of {len(endpoints)} total")
            for ep in selected:
                print(f"  - {ep.method} {ep.path} [{ep.api}] \u2014 {ep.summary}")
            print("=" * 60 + "\n")

        return selected

    # ------------------------------------------------------------------
    # Step 5c: Format a single endpoint with full param/return detail
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Step 5d-helper: Re-evaluate unsatisfied inputs from current chain
    # ------------------------------------------------------------------
    def _compute_unsatisfied(self, chain: List["ChainStep"]) -> List[str]:
        """Deterministically find unresolved URL path parameters across the chain."""
        unsatisfied = []
        for i, cs in enumerate(chain):
            params_provided = {
                prior.satisfies_param for prior in chain[:i] if prior.satisfies_param
            }
            path_params = re.findall(r'\{(\w+)\}', cs.endpoint.path)
            for param in path_params:
                if param in cs.literal_args or param in params_provided:
                    continue
                unsatisfied.append(param)
        return unsatisfied

    def _compute_unsatisfied_detail(self, chain: List["ChainStep"]) -> List[str]:
        """Like _compute_unsatisfied but includes endpoint context and parameter
        description so the resolver loop picks semantically correct endpoints."""
        details = []
        for i, cs in enumerate(chain):
            params_provided = {
                prior.satisfies_param for prior in chain[:i] if prior.satisfies_param
            ...
            for param in path_params:
                if param in cs.literal_args or param in params_provided:
                    continue
                # Look up the parameter description from the swagger spec
                param_desc = ""
                for p in (cs.endpoint.parameters or []):
                    if p.get("name") == param:
                        param_desc = (p.get("description") or "")[:100]
                        break
                entry = f"'{param}' for {cs.endpoint.method} {cs.endpoint.path}"
                if param_desc:
                    entry += f" — {param_desc}"
    ...
    # ------------------------------------------------------------------
    # Step 5e: Build backward chain from goal endpoint using while loop
    # Returns (chain, capabilities) where chain is ordered for execution.
    # ------------------------------------------------------------------
    async def _build_endpoint_chain(self, task: str, relevant: List[EndpointInfo], prior_issues: list = None):
        hints = self._load_hints()
        api_files_used = {ep.api for ep in relevant}
        relevant_hints = [
            f"[ {fname} ]\n{hints[fname]}"
            for fname in api_files_used
            if fname in hints
        ...
        detailed_list = "\n\n".join(
            ...
            for i, ep in enumerate(relevant)
        )

        prior_issues_section = (
            "\nA previous planning attempt failed with these issues — ensure the new chain "
        ...
        # Call 1: identify goal endpoint and its unsatisfied inputs
        goal_prompt = (
            f"Task: {task}\n\n"
            f"Available endpoints:\n{detailed_list}\n\n"
            + api_hints_section
            ...
            "- If the task asks for specific data about a resource, "
            "the goal is the endpoint that fetches that specific resource, "
            "NOT a search or list endpoint.\n"
            ...
            "- Write/mutate endpoints (POST, PUT, DELETE, PATCH) are the goal when the task asks to create, update, or delete something.\n\n"
            "Also list any arguments for the goal endpoint that can be filled directly as literals from the task description.\n\n"
            'Respond with ONLY valid JSON:\n'
            '{"goal_index": 2, "literal_args": {"search": "GAN"}}'
        )
        self._debug_print("_build_endpoint_chain goal", prompt=goal_prompt)
        agent = Agent(self.llm, output_type=str)
        result = await agent.run(goal_prompt)
        response = result.output
        self._debug_print("_build_endpoint_chain goal", response=response)
        data = _parse_json(response)

        goal_index = data.get("goal_index", 0)
        if not (0 <= goal_index < len(relevant)):
            raise ValueError(
                f"LLM returned goal_index {goal_index} but only {len(relevant)} endpoints are available."
            )
        goal_step = ChainStep(
            endpoint=relevant[goal_index],
            capability="performs the main action of the task",
            literal_args=data.get("literal_args", {}),
        )
        chain: List[ChainStep] = [goal_step]
        unsatisfied = self._compute_unsatisfied(chain)

        # Call 2 (one-shot): if the goal has unsatisfied path params, ask the LLM
        # to design the complete resolver chain in a single call so it can reason
        # holistically about resource types and multi-hop dependencies.
        if unsatisfied:
            unsatisfied_detail = self._compute_unsatisfied_detail(chain)
            goal_detail = self._format_endpoint_detail(goal_step.endpoint)

            resolver_prompt = (
                f"Task: {task}\n\n"
                f"Goal endpoint (executes LAST):\n{goal_detail}\n"
                f"Goal literal args already known: {goal_step.literal_args}\n"
                f"Unsatisfied path parameters: {unsatisfied_detail}\n\n"
                f"Available endpoints (numbered):\n{detailed_list}\n\n"
                + api_hints_section
                + prior_issues_section
                + "Design the complete list of resolver steps that must execute BEFORE the goal "
                "to satisfy all its path parameters.\n\n"
                "Rules:\n"
                "- List resolvers in EXECUTION ORDER (first to run is first in the list).\n"
                "- Each resolver's output satisfies a path parameter of the step immediately after it.\n"
                "- Match resource types: if the goal needs a project {id}, the resolver must return "
                "project data (e.g. GET /users/{user_id}/projects), NOT user data (e.g. GET /users).\n"
                "- If a resolver itself has unsatisfied path parameters, include a resolver for those "
                "too (earlier in the list).\n"
                "- For each resolver specify:\n"
                "    endpoint_index: integer index from the numbered list above\n"
                "    satisfies_param: the path parameter name it resolves in the NEXT step\n"
                "    literal_args: parameter values taken directly from the task description\n"
                "    capability: one-line description\n"
                "- If no resolvers are needed, return an empty list.\n\n"
                'Respond with ONLY valid JSON:\n'
                '{"resolvers": [{"endpoint_index": 1, "satisfies_param": "user_id", "literal_args": {"search": "Akilesh Kannan"}, "capability": "looks up user by name"}, {"endpoint_index": 3, "satisfies_param": "id", "literal_args": {}, "capability": "lists projects owned by user"}]}'
            )
            self._debug_print("_build_endpoint_chain resolvers", prompt=resolver_prompt)
            result = await agent.run(resolver_prompt)
            response = result.output
            self._debug_print("_build_endpoint_chain resolvers", response=response)
            data = _parse_json(response)

            seen_indices = {goal_index}
            resolver_steps: List[ChainStep] = []
            for r in data.get("resolvers", []):
                ep_index = r.get("endpoint_index")
                if ep_index is None or not (0 <= ep_index < len(relevant)):
                    continue
                if ep_index in seen_indices:
                    continue
                seen_indices.add(ep_index)
                resolver_steps.append(ChainStep(
                    endpoint=relevant[ep_index],
            ...
            chain = resolver_steps + [goal_step]
    ...
    async def _build_plan(self, task: str, chain_steps: List[ChainStep]):
        ...
        # Build pre-wired chain section from ChainStep metadata
        ...
        for i, cs in enumerate(chain_steps):
            ...
            if i > 0:
                ...
                prev_cs = chain_steps[i - 1]
                if prev_cs.satisfies_param:
                    lines.append(f"    {prev_cs.satisfies_param}: reference {{{prev_sid}.result}}")
            if cs.satisfies_param and i < len(chain_steps) - 1:
                next_sid = step_ids[i + 1]
                lines.append(f"    output provides: {cs.satisfies_param} \u2192 to {next_sid} via {{{sid}.result}}")
            wiring_lines.append("\n".join(lines))
        wiring_section = (
    ...
    def _plan_context(self, chain_steps: List[ChainStep]) -> tuple:
        """Return (endpoint_details, api_hints_section, tool_names, bundle) shared
        by both _check_plan and _fix_plan."""
        selected = [cs.endpoint for cs in chain_steps]
        endpoint_details = "\n\n".join(self._format_endpoint_detail(ep) for ep in selected)
        hints = self._load_hints()
        api_files_used = {ep.api for ep in selected}
    ...
    async def _check_plan(
        ...
        """Pure LLM check. Returns (issues: List[str], ok: bool)."""
    ...
    async def _fix_plan(
        ...
        """Ask the LLM to correct a plan given a list of issues. Returns a new plan_result."""
        ...
        # Re-annotate steps with response_schema and base_url routing tags
        selected = [cs.endpoint for cs in chain_steps]
        endpoint_map = {f"{ep.method} {ep.path}": ep for ep in selected}
        _fallback = EndpointInfo(api="", method="", path="", summary="", description="", parameters=[])
        annotated = [
    ...
    async def _verify_plan(
        ...
        """Check the plan and rebuild the chain + plan if issues are found.

        When issues are detected, re-runs _build_endpoint_chain (with the issue
        list as context) and _build_plan so that new endpoints can be introduced.
        Returns (plan_result, chain_steps) — both may be updated.
        """
        MAX_REBUILD_ATTEMPTS = 2

        for attempt in range(MAX_REBUILD_ATTEMPTS):
            issues, ok = await self._check_plan(task, plan_result, chain_steps)
            if ok or not issues:
                return plan_result, chain_steps

            self._debug_print(
                "_verify_plan",
                response=f"attempt {attempt + 1}: rebuilding due to {len(issues)} issue(s): {issues}",
            )

            # Rebuild the full chain with the issue list as guidance
            chain_steps = await self._build_endpoint_chain(
                task, relevant_endpoints, prior_issues=issues
            )
            if not chain_steps:
                raise ValueError("Rebuild produced no chain steps.")
            plan_result = await self._build_plan(task, chain_steps)

        # Final check after last rebuild
        issues, ok = await self._check_plan(task, plan_result, chain_steps)
        if not ok and issues:
            raise ValueError(
                f"Plan verification failed after {MAX_REBUILD_ATTEMPTS} rebuild(s):\n"
                + "\n".join(f"  - {iss}" for iss in issues)
            )
        return plan_result, chain_steps
    ...
    async def plan(self, task: str):
        ...
        relevant_endpoints = await self._filter_relevant_endpoints(task, all_endpoints)
        if not relevant_endpoints:
            raise ValueError("No relevant endpoints found for this task.")

        chain_steps = await self._build_endpoint_chain(task, relevant_endpoints)
        if not chain_steps:
            raise ValueError("LLM selected no endpoints for this task.")

        plan_result = await self._build_plan(task, chain_steps)

        plan_result, chain_steps = await self._verify_plan(
            task, plan_result, chain_steps, relevant_endpoints
        )
